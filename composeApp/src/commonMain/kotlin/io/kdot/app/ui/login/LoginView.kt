...
import io.kdot.app.designsystem.atomic.molecule.IconTitleSubtitleMolecule
import io.kdot.app.designsystem.components.BigIcon
import io.kdot.app.designsystem.components.buttons.BackButton
import io.kdot.app.designsystem.components.buttons.ProgressButton
...
fun LoginScreen(
    ...
    LaunchedEffect(state.loginResultState) {
        val result = state.loginResultState
        if (result is AsyncData.Success) {
            onLoginSuccess()
            vm.handleAction(LoginEvent.Reset)
        }
    }
    LoginView(
        state = state,
